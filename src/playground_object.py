...
class Playground_Object(pygame.sprite.Sprite):
	...
	def check_and_clamp_ip_with_playground(self, playground_rect: pygame.Rect) :
		top_check: bool = playground_rect.top >= self.m_pos.y - self.m_half_size.y
		bottom_check: bool = playground_rect.bottom <= self.m_pos.y + self.m_half_size.y
		left_check: bool = playground_rect.left >= self.m_pos.x - self.m_half_size.x
		right_check: bool = playground_rect.right <= self.m_pos.x + self.m_half_size.x
